...
- **Capote**: apenas nos jogos de 21 pontos (modo Personalizado). Conta como capote em dois casos: se um jogador chegar aos 11 pontos enquanto o adversário continua a 0 (o set termina logo ali), ou se o set correr até ao fim normalmente mas o jogador que perdeu não tiver passado dos 10 pontos (ex: 21-9). Fica assinalado tanto na lista do histórico como no detalhe do jogo, e conta nas estatísticas do jogador (capotes feitos e sofridos), mesmo não sendo critério de vitória.
...
- **Importar cópia de segurança**: escolhe um ficheiro `.json` exportado anteriormente para repor todos os jogadores e histórico. Útil se limpaste a cache do Chrome, mudaste de tablet, ou queres juntar dados de outro dispositivo. Atenção: isto substitui por completo os dados atuais — a app avisa-te e pede confirmação antes de continuar.
